...
public class CreateBlockDialog extends Dialog<String>{
	...
	public CreateBlockDialog() {
		...
		this.setResultConverter(btn -> {
			...
			if (btn == ButtonType.OK && !namestr.isEmpty()) {
				try {
					Path pth = Paths.get(Cache.getBlockDataPath().toString(), namestr + ".json");
					if(Files.exists(pth))
						return null;
					...
					if(addLangKey.isSelected()) {
						Cache.addLangKey("tile." + Cache.getModID() + ":" + namestr + ".name");
